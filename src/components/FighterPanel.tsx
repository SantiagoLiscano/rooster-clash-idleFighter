...
export function FighterPanel({ playerSide, fighter, image, effect }: FighterPanelProps) {
    ...
    return (
        <article className={`fighter-card fighter-card--${playerSide}`} style={{ position: 'relative' }}>

            {/* Animaciones SVG dinámicas */}
            {effect === 'shield' && (
                <img className={`escudo-animacion escudo-${playerSide}`} src="/escudoEdited.svg" alt="Bloqueo" />
            )}
            {effect === 'claws' && (
                <img className="garras-animacion zarpazo-animado" src="/garras.svg" alt="Impacto" />
            )}

            <p className="eyebrow">{playerSide === "player" ? "Player" : "Opponent"}</p>
            <h3>{fighter.name}</h3>
            <div className={effect === 'dodge' ? "esquivar-animado" : ""}>
                <VisualGallo type={image} color={fighter.color} isRight={playerSide === 'opponent'} />
            </div>
            <div className="health">
                <div className="health__track">
                    <div className="health__bar" style={{ width: `${percentage}%`, background: healthBackground }} />
                </div>
                <p>{Math.max(0, fighter.hp)}/{fighter.maxHp} HP</p>
            </div>
